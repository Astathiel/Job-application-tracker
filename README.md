...
1. **Launch the Application:** Start the application. It will automatically check for an existing `data.json` file and load your saved applications into the grid. If it is your first time opening the app, it will create an empty list.
2. **Add a New Entry:** Fill out the input fields with the details of your job application, including the company name, role, location, work form, application date, and status. 
3. **Save the Data:** Press the 'Save' button. The application will validate that the required fields are filled. If successful, it creates the new entry, overwrites the local JSON file with the updated data, and refreshes the UI.
4. **Edit or Delete:** To modify your saved data, click the pencil (edit) or trash can (delete) icons located on the right side of each row in the data grid.
